...
## Parser-level G76 threading cycle

FluidNC supports a conservative single-line lathe threading cycle dialect: `G76 X<final-x> Z<final-z> P<passes> F<pitch>`. `X` and `Z` are required final positions, `P` is an integer pass count from 1 through 255, and `F` is the thread pitch in the active units per spindle revolution. `G76` requires lathe threading to be enabled, `G95` feed-per-revolution mode, an active spindle, and healthy measured RPM/index/angular feedback. The parser expands the cycle into the same synchronized threading primitives used by `G32/G33`, with rapid retract/return moves between passes.

## Fallback WebUI lathe operator screen

The embedded fallback tool page includes a Lathe Operator Dashboard backed by `ESP421`, `ESP422`, and `ESP423`. The dashboard refreshes status, shows encoder/threading warnings, provides a tool-table editor for geometry/wear/nose/orientation fields, and provides a touch-off workflow with an explicit radius/diameter X input selector. After saving a tool or running touch-off, the page refreshes `ESP421` so the operator can verify active lathe state and feedback health.

## Remaining parser-level lathe cycles

The parser also supports conservative single-line lathe helper-cycle dialects that expand into validated linear/rapid primitives: `G71 X<final-x> Z<end-z> Q<depth-step> F<feed> [L1]` for rough turning/facing, `G70 X<final-x> Z<end-z> F<feed>` for finishing, `G75 X<final-x> Z<groove-z> Q<peck-depth> F<feed>` for grooving, and `G83 [X<x>] Z<final-z> Q<peck-depth> F<feed>` for peck drilling/boring. These cycles are one-shot, require lathe mode, validate their required geometry before motion, and reset back to `G1` after expansion.
